...
<div class="main_sub wrap_w">
	...
	<span></span><a href="<?php echo U('Article/index?type='.$type);?>">新闻中心</a></div>
    <div id="newlist" class="clearfix">
        <div class="l">
            <div class="">
				<ul class="tabNavigation clearfix">
                    <?php echo W('Category/new_lists', array($category['id'], ACTION_NAME == 'index'));;?>
					<!-- <li><a class="<?php if($type == 50): ?>on<?php endif; ?>" href="<?php echo U('Article/index?type=50');?>">游戏公告</a></li>
					<li><a class="<?php if($type == 49): ?>on<?php endif; ?>" href="<?php echo U('Article/index?type=49');?>">游戏新闻</a></li>
					<li><a class="<?php if($type == 53): ?>on<?php endif; ?>" href="<?php echo U('Article/index?type=53');?>">游戏活动</a></li> -->					
				</ul>
                <div class="ntab" id="ntab01" style="display: block;">
                    <ul>
                    <?php $__CATE__ = D('Category')->getChildrenId($category['id']);$__LIST__ = D('Document')->page(!empty($_GET["p"])?$_GET["p"]:1,10)->lists($__CATE__, '`level` DESC,`id` DESC', 1,true); if(is_array($__LIST__)): $i = 0; $__LIST__ = $__LIST__;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$article): $mod = ($i % 2 );++$i;?><li class="clearfix">
                            <span><a href="<?php echo U('Article/detail?id='.$article['id']);?>"></a></span>
                            <h4><a href="<?php echo U('Article/detail?id='.$article['id']);?>"><?php echo ($article["title"]); ?></a>
                            </h4>
                            <p class="pone"></p>
                            <p class="ptwo"><?php echo (date("Y-m-d H:i:s",$article["update_time"])); ?></p>
