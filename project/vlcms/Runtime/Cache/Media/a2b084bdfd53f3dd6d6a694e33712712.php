...
<div class="menu clearfix">
	...
	<span></span><a href="<?php echo U('Game/gift_list');?>">独家礼包</a></div>
    <div class="gameclass">
        <h2 class="titi">礼包<span>分类</span></h2>
        <ul class="gameclasslist">
            <li><span>热门游戏</span>
                <p> 
					<a href="<?php echo U('Game/gift_list');?>" class="<?php if($game_id == 0 ): ?>curr<?php endif; ?>">不限</a>
					<?php if(is_array($gift_list)): foreach($gift_list as $key=>$gl): ?><a href="<?php echo U('Game/gift_list?game_id='.$gl['game_id'].'');?>" class="<?php if($game_id == $gl[game_id]): ?>curr<?php endif; ?>">
                        <?php echo ($gl["game_name"]); ?>
    ...
    <div id="giftlist">
        ...
        <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li>
                <div class="l">
                    <a href="<?php echo U('Game/game_detail?id='.$vo['game_id'].'');?>">
                        <img src="/project/vlcms<?php echo (get_cover($vo["icon"],'path')); ?>">
                    </a>
                </div>
                <div class="r">
                    <h3><?php echo ($vo["giftbag_name"]); ?></h3>
                    <p class="ct">
                        <?php echo ($vo["desribe"]); ?>
                    </p>
                    <p class="cm">
                        剩余有效期：<?php echo ceil(($vo['end_time']-$vo['start_time'])/86400);;?>天
						
                    </p>
                    <div class="bn">
                        <a onclick="XGUC.get_game_gift(<?php echo ($vo["id"]); ?>, &#39;<?php echo ($vo["giftbag_name"]); ?>&#39;)" target="_blank">
                            领取礼包
                        </a>
                    </div>
                </div>
