...
class ThinkController extends AdminController {
    ...
    public function lists($model = null, $p = 0,$extend=array()){
        ...
        if(!empty($extend['field_time'])){
            if(isset($_REQUEST['time-start']) && isset($_REQUEST['time-end'])){
                $map[$extend['field_time']]  = array('BETWEEN',array(strtotime($_REQUEST['time-start']),strtotime($_REQUEST['time-end'])));
    ...
    public function del($model = null, $ids=null,$prefix='tab_'){
    ...
    public function set_status($Model=CONTROLLER_NAME){
        ...
        $base_event = A('Base','Event');

        $map['id'] = array('in',$ids);
        switch ($status){
            case -1 :
                $base_event->delete($Model, $map, $msg[$msg_type],$field);
                break;
            case 0  :
             
                $base_event->forbid($Model, $map, $msg[$msg_type],$field);
                break;
            case 1  :
                $base_event->resume($Model, $map, $msg[$msg_type],$field);
                break;
            case 2  :
                $base_event->reject($Model, $map, $msg[$msg_type],$field);
                break;
            default :
                $this->error('参数错误');
                break;
